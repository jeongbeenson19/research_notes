...
def solution(jobs):  
    # 1) 도착 시각 기준 정렬  
    jobs.sort()  # (start, duration)  
    n = len(jobs)  
      
    time = 0          # 현재 시각  
    i = 0             # 아직 큐에 못 넣은 작업의 인덱스  
    total_wait = 0    # 총 대기시간(= 완료시각 - 도착시각)의 합  
    heap = []         # (duration, start)  
    while i < n or heap:  
        # 2) 현재 시각까지 도착한 작업을 모두 힙에 넣기  
        while i < n and jobs[i][0] <= time:  
            start, dur = jobs[i]  
            heapq.heappush(heap, (dur, start))  
            i += 1  
        if heap:  
            # 3) 가장 짧은 작업 처리  
            dur, start = heapq.heappop(heap)  
            time += dur  
            total_wait += time - start  # 완료시각 - 도착시각  
        else:  
            # 4) 처리할 게 없으면 다음 작업 도착 시각으로 점프  
            time = jobs[i][0]  
