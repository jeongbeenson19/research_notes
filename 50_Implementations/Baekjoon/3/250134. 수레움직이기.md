...
def solution(maze):  
    ...
    for i in range(H):  
        for j in range(W):  
            if maze[i][j] == 1: r0 = (i,j)  
            elif maze[i][j] == 2: b0 = (i,j)  
            elif maze[i][j] == 3: r_goal = (i,j)  
            elif maze[i][j] == 4: b_goal = (i,j)  
      
    red_vis = [[False]*W for _ in range(H)]  
    blue_vis = [[False]*W for _ in range(H)]  
    red_vis[r0[0]][r0[1]] = True  
    blue_vis[b0[0]][b0[1]] = True  
  
    ans = [float('inf')]  
  
    def dfs(rr, rc, br, bc, turns):  
        # 둘 다 도착하면 정답 갱신  
        if (rr, rc) == r_goal and (br, bc) == b_goal:  
            ans[0] = min(ans[0], turns)  
            return  
        # 백트래킹 가지치기  
        if turns >= ans[0]:  
            return  
        for dr_r, dc_r in dirs:  
            ...
            if (rr, rc) == r_goal:  
                ...
                nr, nc = rr + dr_r, rc + dc_r  
                if not (0 <= nr < H and 0 <= nc < W) or maze[nr][nc] == 5 or red_vis[nr][nc]:  
                    continue  
                moved_red = True  
            ...
            for dr_b, dc_b in dirs:  
                ...
                if (br, bc) == b_goal:  
                    ...
                    nb_r, nb_c = br + dr_b, bc + dc_b  
                    if not (0 <= nb_r < H and 0 <= nb_c < W) or maze[nb_r][nb_c] == 5 or blue_vis[nb_r][nb_c]:  
                        continue  
                    moved_blue = True  
                ...
                # 방문 표시  
                if moved_red: red_vis[nr][nc] = True  
                if moved_blue: blue_vis[nb_r][nb_c] = True  
  
                dfs(nr, nc, nb_r, nb_c, turns+1)  
  
                # 방문 복구  
                if moved_red: red_vis[nr][nc] = False  
                if moved_blue: blue_vis[nb_r][nb_c] = False  
  
    dfs(r0[0], r0[1], b0[0], b0[1], 0)  
