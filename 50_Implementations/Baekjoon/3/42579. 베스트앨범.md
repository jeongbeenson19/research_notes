...
def solution(genres, plays):  
    ...
    genre_count_list = defaultdict(int)  
    genre_list = defaultdict(list)  
  
    for i, (g, p) in enumerate(zip(genres, plays)):  
        genre_count_list[g] += p  
        genre_list[g].append((p, i))  
          
  
    sorted_genre = sorted(genre_count_list, key=lambda x: genre_count_list[x], reverse=True)  
      
    answer = []  
      
  
    for g in sorted_genre:  
        songs = sorted(genre_list[g], key=lambda x: (-x[0], x[1]))  
