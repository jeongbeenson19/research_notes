...
for n in range(reps):  
    dx, dy, ax, ay = map(int, sys.stdin.readline().split())  
    sys_num = int(sys.stdin.readline())  
    result = 0  
    for m in range(sys_num):  
        cx, cy, r = map(int,sys.stdin.readline().split())  
        start = (dx-cx)**2+(dy-cy)**2 < r**2  
        end = (ax-cx)**2+(ay-cy)**2 < r**2  
        if start != end:  
            result += 1  
    print(result)
```
