...
def solution(video_len, pos, op_start, op_end, commands):  
    vlen_a, vlen_b = map(int, video_len.split(':'))  
    pos_a, pos_b = map(int, pos.split(':'))  
    op_start_a, op_start_b = map(int, op_start.split(':'))  
    op_end_a, op_end_b = map(int, op_end.split(':'))  
    video_len = vlen_a*60+vlen_b  
    pos=pos_a*60+pos_b  
    op_start=op_start_a*60+op_start_b  
    op_end=op_end_a*60+op_end_b  
      
    if op_start <= pos <= op_end:  
        pos = op_end  
    print(f"{op_start}," + f"{pos},"+ f"{op_end}")  
