...
구현, 자료 구조, 시뮬레이션, [[Queue|덱]], 큐  
  
### 제출 일자  
  
2025년 8월 10일 16:24:48  
...
for _ in range(K):  
    ...
    board[r-1][c-1] = 1  # 입력은 1-index, 보드는 0-index  
L = int(input())  
turn_at = {}  # 시간 -> 'L' or 'D'for _ in range(L):  
    t, d = input().split()  
    turn_at[int(t)] = d  
...
snake = deque([(0, 0)])         # 머리가 맨앞  
occupied = set([(0, 0)])        # 충돌 체크용  
time = 0  
...
while True:  
    ...
    # 사과 유무  
    if board[ny][nx] == 1:  
        board[ny][nx] = 0  # 길이 +1 (꼬리 유지)  
    else:  
        ty, tx = snake.pop()   # 길이 유지 (꼬리 한 칸 줄이기)  
        occupied.remove((ty, tx))  
